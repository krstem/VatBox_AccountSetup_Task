...
const ActivationDataFormatter = ({ row }) => {
    ...
    if (row.activationData <= 50) {
        return <div style={{ "width": "100px" }}><ProgressBarIndicator percent={row.activationData} height={'8px'} color={'#fe645f'} /></div>
    }

    if (row.activationData > 50) {
        return <div style={{ "width": "100px" }}><ProgressBarIndicator percent={row.activationData} height={'8px'} color={'#ffac1c'} /></div>
    }
};
