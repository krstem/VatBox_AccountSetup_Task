...
const PaymentSetupFormatter = ({row}) => {
    ...
    if(row.paymentSetup <= 50) {
        return <div style={{"width": "100px"}}><ProgressBarIndicator percent={row.paymentSetup} height={'8px'} color={'#fe645f'}/></div>
    }

    if(row.paymentSetup > 50) {
        return <div style={{"width": "100px"}}><ProgressBarIndicator percent={row.paymentSetup} height={'8px'} color={'#ffac1c'}/></div>
    }
};
